...
- Add visual regression tests for popup UI states.
- Introduce Playwright smoke runs against unpacked extension for navigation and modal flows.
- Wire optional `@web/worker-tools` or Comlink for clearer worker messaging without tight coupling.

Stay aligned to this stack plan to keep MarketPulseAI fully local, predictable, and ready for fast iteration.

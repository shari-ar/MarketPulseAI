...
async function resolveTabNavigator() {
  if (globalThis.TabNavigator) {
    return globalThis.TabNavigator;
  }

  try {
    const module = await import("./navigation/tabNavigator.js");
    return module?.TabNavigator ?? module?.default ?? null;
  } catch (error) {
...
async function createNavigator() {
  const TabNavigator = await resolveTabNavigator();
  if (!TabNavigator) {

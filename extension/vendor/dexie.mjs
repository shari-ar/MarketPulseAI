...
derive(DexieError)
  ...
  .extend({
    stack: {
      get: function () {
        return (
          this._stack || (this._stack = this.name + ": " + this.message + prettyStack(this._e, 2))
        );
      },
    },
    toString: function () {
      ...
      setProp(reason, "stack", {
        get: function () {
          return stack_being_generated
            ? origProp && (origProp.get ? origProp.get.apply(reason) : origProp.value)
            : promise.stack;
        },
      });
    });
...
var Table = (function () {
  ...
  Table.prototype.get = function (keyOrCrit, cb) {
    ...
    return this._trans("readonly", function (trans) {
      return _this.core.get({ trans: trans, key: keyOrCrit }).then(function (res) {
  ...
  Table.prototype.orderBy = function (index) {
    return new this.db.Collection(
      new this.db.WhereClause(this, isArray(index) ? "[" + index.join("+") + "]" : index)
    );
  };
  ...
  Table.prototype.update = function (keyOrObject, modifications) {
    if (typeof keyOrObject === "object" && !isArray(keyOrObject)) {
      ...
      if (key === undefined)
        return rejection(
          new exceptions.InvalidArgument("Given object does not contain its primary key")
        );
      try {
  ...
  Table.prototype.bulkAdd = function (objects, keysOrOptions, options) {
    ...
    return this._trans("readwrite", function (trans) {
      ...
      var objectsToAdd =
        keyPath && auto ? objects.map(workaroundForUndefinedPrimKey(keyPath)) : objects;
      return _this.core
        .mutate({
          trans: trans,
          type: "add",
        ...
        .then(function (_a) {
          ...
          throw new BulkError(
            _this.name + ".bulkAdd(): " + numFailures + " of " + numObjects + " operations failed",
            failures
          );
        });
    });
    ...
    return this._trans("readwrite", function (trans) {
      ...
      return _this.core
        ...
        .then(function (_a) {
          ...
          if (numFailures === 0) return result;
          throw new BulkError(
            _this.name + ".bulkPut(): " + numFailures + " of " + numObjects + " operations failed",
            failures
          );
        });
    });
  ...
  Table.prototype.bulkDelete = function (keys) {
    ...
    return this._trans("readwrite", function (trans) {
      ...
      throw new BulkError(
        _this.name + ".bulkDelete(): " + numFailures + " of " + numKeys + " operations failed",
        failures
      );
    });
  };
...
function getIndexOrStore(ctx, coreSchema) {
  ...
  if (!index)
    throw new exceptions.Schema(
      "KeyPath " + ctx.index + " on object store " + coreSchema.name + " is not indexed"
    );
  return index;
}
  ...
  return makeClassConstructor(
    ...
    function Collection(whereClause, keyRangeGenerator) {
      ...
      this._ctx = {
        ...
        isPrimKey:
          !whereCtx.index ||
          (table.schema.primKey.keyPath && whereCtx.index === table.schema.primKey.name),
        range: keyRange,
        keysOnly: false,
        dir: "next",
        unique: "",
  ...
  WhereClause.prototype.inAnyRange = function (ranges, options) {
    ...
    if (
      !ranges.every(function (range) {
        return (
          range[0] !== undefined && range[1] !== undefined && ascending(range[0], range[1]) <= 0
        );
      })
    ) {
      return fail(
...
function createTransactionConstructor(db) {
  return makeClassConstructor(
    ...
    function Transaction(mode, storeNames, dbschema, chromeTransactionDurability, parent) {
      var _this = this;
      this.db = db;
      this.mode = mode;
          ...
          mutate: function (req) {
                  ...
                  .then(function (_a) {
                    ...
                    for (var i = 0; i < keys.length; ++i) {
                      ...
                      if (primKey == null) {
                        ...
                        ctx.onsuccess &&
                          ctx.onsuccess(
                            req.type === "put" && existingValues[i] ? req.values[i] : primKey
                          );
                      }
                    }
                    return {
            ...
            function deleteNextChunk(trans, range, limit) {
              return downTable
                ...
                .then(function (_a) {
                  var result = _a.result;
                  return addPutOrDelete({ type: "delete", keys: result, trans: trans }).then(
                    function (res) {
...
function addRange(target, from, to) {
  ...
  if (cmp(to, target.from) < 0) {
    left ? addRange(left, from, to) : (target.l = { from: from, to: to, d: 1, l: null, r: null });
    return rebalance(target);
  }
  if (cmp(from, target.to) > 0) {
    right ? addRange(right, from, to) : (target.r = { from: from, to: to, d: 1, l: null, r: null });
    return rebalance(target);
  }
...
var symbolObservable =
  typeof Symbol !== "undefined" && "observable" in Symbol ? Symbol.observable : "@@observable";
var Observable = (function () {
  ...
  Observable.prototype.subscribe = function (x, error, complete) {
    return this._subscribe(
      !x || typeof x === "function" ? { next: x, error: error, complete: complete } : x
    );
  };
...
try {
  domDeps = {
    indexedDB:
      _global.indexedDB || _global.mozIndexedDB || _global.webkitIndexedDB || _global.msIndexedDB,
    ...
    try {
      if (!propagatingLocally) {
        ...
        if (typeof self["clients"] === "object") {
          __spreadArray([], self["clients"].matchAll({ includeUncontrolled: true }), true).forEach(
            function (client) {
